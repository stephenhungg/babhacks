...
The agent holds the fulfillment preimage. When a conversion event is confirmed (e.g., MetaLex webhook), the agent calls `EscrowFinish` with the fulfillment. This makes the agent a **trustless trustee**.
